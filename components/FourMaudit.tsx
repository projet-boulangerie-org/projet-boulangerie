...
export default function LeFourMaudit() {
    ...
    setLives((v) => {
      ...
      if (left <= 0) {
        ...
        setBest((b) => {
          ...
          if (typeof window !== "undefined")
            localStorage.setItem("bakeryBest", String(nb));
          return nb;
        });
      } else {
        toast(reason);
  ...
  // ------- dérivés
  const timePct = order
    ? Math.max(0, Math.min(100, (timeLeftMs / order.timeMs) * 100))
    : 0;
      ...
      <main className="flex-1 flex flex-col items-center text-white px-4">
        {!mounted ? (
          ...
          <>
            ...
            {/* Titre */}
            <h2 className="text-4xl font-medieval text-boulange-gold mt-2 mb-16">
              Boulangerie Rush
            </h2>
            ...
            {!gameOver && (
              <div className="w-full max-w-3xl bg-white/5 rounded-2xl p-5 border border-white/10">
                <div className="rounded-xl bg-black/30 border border-white/10 p-6">
                        ...
                        >
                          ...
                          <div className="mt-1 text-xs">
                            ...
                            <span
                              className={
                                isGood
                                  ? "font-semibold text-boulange-gold"
                                  : "font-semibold"
                              }
                            >
                              {needLeft}
                  ...
                  {/* Message */}
                  {msg && (
                    <p className="mt-4 text-center italic text-gray-300 min-h-6">
                      {msg}
                    </p>
                  )}
                </div>
